//! UI-facing application data shared by the desktop views.
//!
//! This module intentionally contains no toolkit types.  The old Druid UI keeps
//! these values in `ui.rs`; keeping the model here lets a GPUI view consume the
//! same data while the command/event plumbing is migrated separately.
...
mod tests {
    ...
    fn browser(name: &str, restriction: Option<&str>) -> UIBrowser {
        let matchers = restriction
            .map(|rule| vec![crate::url_rule::to_url_matcher(rule).to_glob_matcher()])
            .unwrap_or_default();
        UIBrowser {
            browser_profile_index: 0,
            is_first: false,
            is_last: false,
            restricted_url_matchers: Arc::new(matchers),
            browser_name: name.to_owned(),
            profile_name: "Default".to_owned(),
            profile_name_maybe: None,
            supports_profiles: false,
            supports_incognito: false,
            icon_path: String::new(),
            profile_icon_path: String::new(),
            unique_id: name.to_owned(),
            unique_app_id: name.to_owned(),
            filtered_index: 0,
        }
    }

    #[test]
    fn restricted_profiles_are_filtered_and_sorted_first() {
        let browsers = Arc::new(vec![
            browser("ordinary", None),
            browser("work", Some("example.com")),
        ]);
        let filtered = get_filtered_browsers("https://example.com/path", &browsers);
        assert_eq!(
            filtered
                .iter()
        ...
        assert_eq!(filtered[0].filtered_index, 0);
        assert_eq!(filtered[1].filtered_index, 1);
    }

    #[test]
    fn restricted_profiles_do_not_show_for_invalid_or_unmatched_urls() {
        let browsers = Arc::new(vec![browser("work", Some("example.com"))]);
        assert!(get_filtered_browsers("not a url", &browsers).is_empty());
        assert!(get_filtered_browsers("https://other.test/path", &browsers).is_empty());
    }
}
